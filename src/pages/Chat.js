...
import ScrollToBottom,{useScrollToBottom,useScrollTo} from 'react-scroll-to-bottom';
...
function Chat() {
 ...
 var id = lo[lo.length - 1];
    ...
    const handleSendChat=(e)=>{
        e.preventDefault()
      let newTranscript = { id:chance.guid(),
        alias:'you',
    ...
    if (chat.length != 0) {
        return (
            <Flex height='100%' flexDirection='column'  overflow='scroll' sx={{position:'relative'}}>
                <Box bg="#F2F2F2" p={2}>
                    <Flex alignItems='center' width='100%'>
                        <Box>
                            <Image src={chat[0].avatar} width={80} sx={{ borderRadius: '100%' }} />
                        </Box>
                        <Box ml={2}>
                ...
                <Flex id="scrollableDiv" p={3} sx={{overflow:'scroll',
            display: 'flex',
        ...
        <ScrollToBottom className={ROOT_CSS}>
                        ...
                        chat[0].transcript.map((c,i) => {
                            ...
                            if(c.alias==='you'){
                                return(
                                    <Box ref={myRef} key={i}  >
                                        {c.message.img?
                                         <Box sx={{ bg: 'rgba(204, 241, 254, 0.5)', borderRadius: '10px' ,minWidth:'10%',maxWidth:'40%', width:'100%',textAlign:'left'}} mt={2} mr={0} ml='auto' p={2}>
                                         <Box>
                                    ...
                                    <Box key={c.id}  sx={{ bg: 'rgba(204, 241, 254, 0.5)', borderRadius: '10px' ,width:'40%',textAlign:'left'}} mt={2} mr={0} ml='auto' p={2}>

                                    <Text fontSize={'12px'} color='secondary'>{c.alias}</Text>
                            ...
                            return (
                                <Box key={c.id} sx={{ bg: '#fff', borderRadius: '10px' ,width:'40%'}} mt={2} p={2}>
                                    <Text fontSize={'12px'} color='secondary'>{c.alias}</Text>
                                    <Text>{c.message.text}</Text>
                                </Box>
                            )
                ...
                <Input
                sx={{bg:'#fff',width:'70%',border:'none',borderRadius:'20px'}}
        id='name'
        name='name'
        value={newMessage}
